...
// Get favorite games
$favoriteGamesQuery = "SELECT og.*, p.name, p.image 
                      FROM owned_games og
                      JOIN products p ON og.product_id = p.id
                      WHERE og.user_id = ? AND og.is_favorite = 1";
$favoriteStmt = $conn->prepare($favoriteGamesQuery);
$favoriteStmt->bind_param("i", $userId);
$favoriteStmt->execute();
$favoriteGamesResult = $favoriteStmt->get_result();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    if (isset($_FILES['profile_image']) && $_FILES['profile_image']['size'] > 0) {
        ...
        if ($check !== false) {
            ...
            if ($_FILES["profile_image"]["size"] <= 5000000) {
                ...
                $updateMessage = "Sorry, your file is too large. Maximum size is 5MB.";
            }
        } else {
    ...
    $updateQuery = "UPDATE users SET username = ?, phone = ?, address = ?, description = ?, profile_image = ? WHERE id = ?";
    $updateStmt = $conn->prepare($updateQuery);
    $updateStmt->bind_param("sssssi", $username, $phone, $address, $description, $profileImage, $userId);
...
<div class="profile-container">
    ...
    <div class="profile-content">
        <div class="profile-sidebar">
            <div class="profile-image-container">
                <?php if (!empty($user['profile_image'])): ?>
                    <img src="uploads/profiles/<?php echo htmlspecialchars($user['profile_image']); ?>" alt="Profile Image" class="profile-image">
                <?php else: ?>
        ...
        <?php if($favoriteGamesResult->num_rows > 0): ?>
        <div class="favorite-games">
            <h3>My Favorite Games</h3>
            <div class="games-grid">
                <?php while($game = $favoriteGamesResult->fetch_assoc()): ?>
                    <div class="game-card">
                        <div class="game-image">
                            <?php if(!empty($game['image'])): ?>
                                <img src="uploads/<?php echo htmlspecialchars($game['image']); ?>" alt="<?php echo htmlspecialchars($game['name']); ?>">
                            <?php else: ?>
        ...
        <div id="profile-form-container" class="profile-form-container" style="display: none;">
            <form method="POST" enctype="multipart/form-data" class="profile-form">
                ...
                <div class="form-group">
                    ...
                    <textarea id="description" name="description" rows="4" placeholder="Share something about yourself..."><?php echo htmlspecialchars($user['description'] ?? ''); ?></textarea>
                </div>
                ...
                <div class="form-group">
                    <button type="submit" name="update_profile" class="update-button">Save Changes</button>
